-- =====================================================================
-- ITFA I-SMS  ·  LMS (Classroom) feature — LIVE DATABASE integration
-- Creates 21 classroom_* tables. Safe to re-run (CREATE IF NOT EXISTS).
-- Generated from the verified local schema on 2026-07-25.
-- Run in phpMyAdmin (SQL tab) on the live enrollment_db.
-- =====================================================================
...
-- ---------------------------------------------------------------------
-- OPTIONAL: mark the Laravel migration as already applied, so a future
-- `php artisan migrate` on the server does NOT recreate these tables.
-- Only run if the portal uses Laravel migrations on live. Ignore the
-- error if a `migrations` table does not exist yet.
-- ---------------------------------------------------------------------
INSERT INTO migrations (migration, batch)
SELECT '2026_07_25_002113_create_lms_tables',
       COALESCE((SELECT MAX(batch) FROM migrations), 0) + 1
FROM DUAL
WHERE NOT EXISTS (
    SELECT 1 FROM migrations WHERE migration = '2026_07_25_002113_create_lms_tables'
);
