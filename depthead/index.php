...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'add_class') {
            ...
            if ($semesterId <= 0 || $strandId <= 0 || $gradeLevelId <= 0 || $sectionId <= 0 || $subjectId <= 0 || $teacherId <= 0) {
            ...
            $insStmt->bind_param(
                ...
                $sectionId, $subjectId, $teacherId, $time, $status, $dhUserId
            );
            $insStmt->execute();
...
if ($selectedSectionId > 0 && $activeSchoolYearId > 0) {
    try {
        ...
        $clStmt->execute();
        $classes = stmt_fetch_all_assoc($clStmt);
    } catch (Throwable) {}
}

// Stats: classes managed by this dept head for the active year
$myTotalClasses = 0;
try {
    $stStmt = $connection->prepare(
        'SELECT COUNT(*) AS cnt FROM classes WHERE School_year_id = ? AND user_id = ?'
    );
    $stStmt->bind_param('ii', $activeSchoolYearId, $dhUserId);
    $stStmt->execute();
    $myTotalClasses = (int) (stmt_fetch_assoc($stStmt)['cnt'] ?? 0);
} catch (Throwable) {}

$mySections = 0;
try {
    $stStmt2 = $connection->prepare(
        'SELECT COUNT(DISTINCT Section_id) AS cnt FROM classes WHERE School_year_id = ? AND user_id = ?'
    );
    $stStmt2->bind_param('ii', $activeSchoolYearId, $dhUserId);
    $stStmt2->execute();
    $mySections = (int) (stmt_fetch_assoc($stStmt2)['cnt'] ?? 0);
} catch (Throwable) {}
...
<head>
    ...
    <title>Class Scheduling | Dept Head Portal</title>
...
<div class="min-h-screen lg:grid lg:grid-cols-[280px_1fr]">
    ...
    <main class="p-4 sm:p-6 lg:p-8 bg-[radial-gradient(circle_at_top,_rgba(22,101,52,0.12),_rgba(241,245,249,0.86)_45%,_rgba(241,245,249,1)_78%)]">
        ...
        <header class="bg-white/90 backdrop-blur rounded-3xl border border-green-100 shadow-panel p-6 mb-6">
            ...
            <p class="text-slate-500 mt-2">Build and manage the class schedule for your sections. Assign subjects, teachers, and time slots. The Registrar will use this to generate each student's individual schedule upon enrollment confirmation.</p>
        </header>
        ...
        <section class="grid gap-4 sm:grid-cols-3 mb-6">
            ...
            <article class="rounded-2xl bg-white border border-green-200 p-5 shadow-panel">
                <p class="text-sm text-green-700">My Sections</p>
                <p class="text-3xl font-extrabold mt-2 text-green-800"><?= h((string) $mySections) ?></p>
                <p class="text-xs text-slate-400 mt-1">sections you manage</p>
            </article>
            <article class="rounded-2xl bg-white border border-slate-200 p-5 shadow-panel">
                <p class="text-sm text-slate-600">My Classes</p>
                <p class="text-3xl font-extrabold mt-2"><?= h((string) $myTotalClasses) ?></p>
                <p class="text-xs text-slate-400 mt-1">class entries you've added</p>
            </article>
        </section>

        <!-- Section + Semester Filter -->
        <form method="GET" action="index.php" class="mb-6 bg-white rounded-2xl border border-slate-200 p-5 shadow-panel flex flex-wrap gap-4 items-end">
        ...
        <div class="mb-4 flex items-center justify-between flex-wrap gap-3">
            ...
            <div class="flex items-center gap-2 flex-wrap">
                <a href="<?= h(app_url('depthead/preview.php') . '?section_id=' . $selectedSectionId . '&semester_id=' . $selectedSemesterId) ?>"
                   class="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-colors flex items-center gap-2">
                            ...
                            <tr class="hover:bg-green-50/40 transition-colors">
                                ...
                                <td class="px-4 py-3 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <a href="<?= h(app_url('depthead/preview.php') . '?section_id=' . $selectedSectionId . '&semester_id=' . $selectedSemesterId . '&class_id=' . (int) $cls['Class_id']) ?>"
        ...
        <div class="rounded-3xl border border-dashed border-slate-300 bg-white/60 p-12 text-center text-slate-400">
            ...
            <p class="font-semibold text-slate-500">Select a grade level and section above to view or manage its schedule.</p>
        </div>
        <?php endif; ?>
        ...
        <form method="POST" action="index.php" class="p-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="time" placeholder="e.g. 7:30-8:30 (Tuesday)"
                       autocomplete="off"
                       class="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-400">
                <p class="text-xs text-slate-400 mt-1">e.g. 7:30-8:30 (Tuesday)</p>
            </div>
...
<div id="deleteModal" style="display:none" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
    <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md">
        <div class="p-6 border-b border-slate-100">
            ...
            <p class="text-sm text-slate-500 mt-1">This will permanently remove the class from the schedule.</p>
        </div>
        <div class="p-6">
            <p class="text-sm text-slate-700 mb-6">
                Remove <strong id="deleteSubjectName" class="text-slate-900"></strong> from the schedule?
            </p>
            <form method="POST" action="index.php" class="flex gap-3">
                <input type="hidden" name="csrf_token"  value="<?= h(csrf_token()) ?>">
...
function teacherLabel(t) {
    ...
    // Flag them so the head knows to reassign.
...
// ── Instances (initialised in DOMContentLoaded) ───────────────────────────────
...
function openEditModal(data) {
    ...
    // Build subject options for this grade + semester
...
window.addEventListener('DOMContentLoaded', () => {    console.log('[DeptHead] allSubjects loaded:', allSubjects.length, '| addGradeLevel:', document.getElementById('addGradeLevel').value);    addSubjectSS  = new SearchableSelect('addSubjectSS',  buildAddSubjectOptions());
