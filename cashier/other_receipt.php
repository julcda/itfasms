...
$id  = to_int($_GET['id'] ?? 0);
$pay = $id > 0 && op_schema_ready($connection) ? op_get($connection, $id) : null;
if (!$pay) {
    flash_set('error', 'Receipt not found.');
    redirect_to(app_url('cashier/other_payment.php'));
}
$items = op_items($connection, $id);

$total       = (float) $pay['Amount'];
$amountWords = peso_in_words($total);
$when        = date('F j, Y g:i A', strtotime((string) ($pay['created_at'] ?: $pay['Date'])));
$isVoid      = $pay['status'] === 'Void';
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .meta .l { color: #64748b; display: inline-block; min-width: 60px; }
...
<body onload="window.print()">
    <div class="toolbar">
        ...
        <a href="<?= h(app_url('cashier/other_payment.php')) ?>">Back</a>
    </div>
    ...
    <div class="slip">
        ...
        <div class="orline">
            ...
            <span><?= h(date('M j, Y', strtotime((string) ($pay['created_at'] ?: $pay['Date'])))) ?></span>
        </div>

        <div class="meta">
            <div><span class="l">Received from:</span> <strong><?= h((string) $pay['Name']) ?></strong></div>
            <div><span class="l">Payment:</span> <?= h((string) $pay['payment_method']) ?><?php if ($pay['reference_no']): ?> · Ref <?= h((string) $pay['reference_no']) ?><?php endif; ?><?php if ($pay['school_year']): ?> · S.Y. <?= h((string) $pay['school_year']) ?><?php endif; ?></div>
        </div>

        <table>
            <thead><tr><th>Item / Purpose</th><th class="r">Qty</th><th class="r">Unit</th><th class="r">Amount</th></tr></thead>
            <tbody>
                <?php foreach ($items as $it): ?>
                <tr>
                    <td><?= h((string) $it['item_name']) ?></td>
                    <td class="r"><?= (int) $it['quantity'] ?></td>
                    <td class="r">₱<?= number_format((float) $it['unit_amount'], 2) ?></td>
                    <td class="r">₱<?= number_format((float) $it['amount'], 2) ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
            <tfoot>
                <tr><td colspan="3" class="r">TOTAL</td><td class="r">₱<?= number_format($total, 2) ?></td></tr>
            </tfoot>
        </table>
